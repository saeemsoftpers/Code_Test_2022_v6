...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs($user_id)
    {
        ...
        if ($cuser) {
            if ($cuser->is('customer')) {
                ...
                $jobs = Job::getTranslatorJobs($cuser->id, 'new')->pluck('jobs');
                $usertype = 'translator';
            }
            ...
            $normalJobs = $normalJobs->each(function ($item, $key) use ($user_id) {
    ...
    public function storeJobEmail(array $data): array
    {
        ...
        $recipientEmail = $job->user_email ?: $user->email;
    ...
    public function jobEnd($postData = [])
    {
        ...
        $sessionTime = $dueDate->diffForHumans($job->end_at, ['parts' => 3]);
        ...
        $translatorJob->completed_at = now();
        $translatorJob->completed_by = $postData['userid'];
        $translatorJob->save();

        event(new SessionEnded($job, $postData['userid'] == $job->user_id ? $translator->id : $job->user_id));
    }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        // Determine the type of jobs to retrieve based on the translator type so we should witch instead of if else
        ...
        switch ($translator_type) {
            case 'professional':
                ...
                break;
            case 'rwstranslator':
                $job_type = 'rws'; // Show only RWS jobs for RWS translators
                break;
            case 'volunteer':
            default:
                $job_type = 'unpaid'; // Show only unpaid jobs for volunteers and any other unspecified translator type
                break;
        }
        ...
        // Get the user's gender and translator level from the metadata
        $gender = $user_meta->gender;
        $translator_level = $user_meta->translator_level;
    
        // Get the job IDs based on the above criteria
        $job_ids = Job::getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
    
        // Filter out jobs that don't match the translator's town
        ...
        // Convert the job IDs into job objects
    ...
    public function sendSMSNotificationToTranslators($job)
    {
        ...
        $dueDate = Carbon::parse($job->due)->format('d.m.Y');
        $dueTime = Carbon::parse($job->due)->format('H:i');
        $duration = $this->convertToHoursMins($job->duration);
        $jobId = $job->id;
        $city = $job->city ?? $jobPosterMeta->city;
    
        // determine message type
        $messageTemplate = ($job->customer_physical_type === 'yes') ? 
            trans('sms.physical_job', ['date' => $dueDate, 'time' => $dueTime, 'town' => $city, 'duration' => $duration, 'jobId' => $jobId]) :
            trans('sms.phone_job', ['date' => $dueDate, 'time' => $dueTime, 'duration' => $duration, 'jobId' => $jobId]);
